...
FORECAST_OFFSET_HOURS = (0, 3, 6, 12)
...
def concise_location_label(primary: str | None, *, query: str) -> str:
    ...
    if len(parts) >= 3 and parts[-1].casefold() == "united states":
        state = parts[-2]
        return parts[0] if state.casefold() == "california" else f"{parts[0]}, {state}"
    if len(parts) >= 2 and parts[1].casefold() not in {"california", "united states"}:
        return f"{parts[0]}, {parts[1]}"
    return parts[0] if parts else primary_clean


def concise_nominatim_location_label(primary: str | None, *, query: str) -> str:
    """Prefer city + state/country from Nominatim display names that begin with the postcode."""
    primary_clean = (primary or "").strip()
    if not primary_clean:
        return query
    parts = [part.strip() for part in primary_clean.split(",") if part.strip()]
...
class WeatherSceneService:
    ...
    def __init__(self) -> None:
        ...
        self.weather_bundle_cached_at: dict[str, datetime] = {}
        self.provider_snapshot_cache: dict[str, WeatherSnapshot] = {}
    ...
    async def resolve_weather_location(
        ...
        *,
        ...
        if not results:
            hinted_geo = await search_nominatim(
                f"{normalized} {country_hint}" if country_hint else normalized
            )
            if hinted_geo:
                latitude = float(hinted_geo["lat"])
                longitude = float(hinted_geo["lon"])
                primary = str(
                    hinted_geo.get("display_name")
                    or hinted_geo.get("name")
                    or normalized
                )
                return LocationRecord(
                    query=normalized,
                    display_name=concise_nominatim_location_label(
                        primary, query=normalized
                    ),
                    latitude=latitude,
                    longitude=longitude,
                    timezone=None,
                    country=None,
                    country_code=country_hint.upper() if country_hint else None,
                    admin1=None,
                    admin2=None,
        ...
        def postcodes(row: dict[str, Any]) -> set[str]:
            return {
                str(code).strip()
                for code in row.get("postcodes") or []
                if str(code).strip()
            }
        ...
        hinted_rows = [
            row
            for row in results
            if str(row.get("country_code") or "").upper()
            == str(country_hint or "").upper()
        ]
        exact_hinted_row = next(
            (row for row in hinted_rows if normalized in postcodes(row)), None
        )
        us_rows = [
            row for row in results if str(row.get("country_code") or "").upper() == "US"
        ]
        exact_us_row = next(
            (row for row in us_rows if normalized in postcodes(row)), None
        )
        if exact_us_row is None and normalized in SF_ZIP_TO_HOOD:
            ...
            exact_row = next(
                (row for row in results if normalized in postcodes(row)), None
            )
            hinted_rows = [
                row
                for row in results
            ...
            us_rows = [
                row
                for row in results
                if str(row.get("country_code") or "").upper() == "US"
            ]
            exact_us_row = next(
                (row for row in us_rows if normalized in postcodes(row)), None
            )
        ...
        geo = (
            ...
            or exact_row
            or (
                hinted_rows[0] if hinted_rows else us_rows[0] if us_rows else results[0]
            )
        )
    ...
    async def fetch_weather_bundle(
        ...
        # Always pull hourly so the forecast buttons resolve from one bundle.
        fetch_hours = max(hours_ahead, max(FORECAST_OFFSET_HOURS))
        async with httpx.AsyncClient(timeout=12.0) as client:
            loc = await self.resolve_weather_location(
                client, zip_code, country_hint=country_hint
            )
            if self.tomorrow_api_key:
                try:
                    return await fetch_tomorrow_bundle(
                        client,
                        loc,
                        api_key=self.tomorrow_api_key,
                        hours_ahead=fetch_hours,
                    )
                except Exception as exc:
                    tomorrow_exc = exc
                    logger.warning(
                        "Tomorrow.io failed (%s); trying Weatherstack.",
                        explain_weather_provider_failure(exc),
                    )
        ...
        raise RuntimeError(
            "No weather provider succeeded. Set TOMORROW_API_KEY and/or WEATHERSTACK_API_KEY."
        )

    def select_hourly_snapshot(
        self, bundle: ForecastBundle, hours_ahead: int
    ) -> WeatherSnapshot:
        ...
        return min(
            bundle.hourly, key=lambda snapshot: abs(snapshot.observed_at - target_time)
        )

    def fresh_cached_bundle(
        self, cache_key: str, hours_ahead: int
    ) -> ForecastBundle | None:
        """Return a fresh cached bundle that can satisfy the requested forecast hour."""
        cached_bundle = self.weather_bundle_cache.get(cache_key)
        cached_at = self.weather_bundle_cached_at.get(cache_key)
        if not cached_bundle or not cached_at:
            return None
        if datetime.now(UTC) - cached_at > WEATHER_CACHE_TTL:
            return None
        if hours_ahead > 0 and not cached_bundle.hourly:
            return None
        return cached_bundle

    def build_scene_from_bundle(
        self, bundle: ForecastBundle, cache_key: str, hours_ahead: int
    ) -> SceneState:
        """Build a scene from a provider bundle or a fresh cached provider bundle."""
        snapshot = (
            bundle.current
            if hours_ahead == 0
            else self.select_hourly_snapshot(bundle, hours_ahead)
        )
        scene = build_scene(
            snapshot,
        ...
        scene.forecast_temps = self.forecast_temps_from_bundle(bundle)
        if hours_ahead == 0:
            self.current_scene_cache[cache_key] = scene
        return scene

    def forecast_temps_from_bundle(self, bundle: ForecastBundle) -> dict[str, float]:
        """Return temperatures for the frontend forecast offsets from one bundle."""
        return {
            str(hours): self.select_hourly_snapshot(bundle, hours).temperature_f
            for hours in FORECAST_OFFSET_HOURS
        }
    ...
    async def get_scene(
        ...
        *,
        ...
        cache_key = zip_code
        try:
            cached_bundle = self.fresh_cached_bundle(cache_key, hours_ahead)
            if cached_bundle:
                return self.build_scene_from_bundle(
                    cached_bundle, cache_key, hours_ahead
                )
            bundle = await self.fetch_weather_bundle(
                zip_code, hours_ahead=hours_ahead, country_hint=country_hint
            )
            self.weather_bundle_cache[cache_key] = bundle
            self.weather_bundle_cached_at[cache_key] = datetime.now(UTC)
            self.provider_snapshot_cache[cache_key] = bundle.current
            return self.build_scene_from_bundle(bundle, cache_key, hours_ahead)
            ...
            if cached_bundle and (hours_ahead == 0 or cached_bundle.hourly):
                ...
                scene = build_scene(
                    snapshot,
                    self.current_scene_cache.get(cache_key)
                    if hours_ahead == 0
                    else None,
                    mode="forecast" if hours_ahead > 0 else "current",
                    hours_ahead=hours_ahead,
                ...
                return scene
            raise RuntimeError(
                f"Live weather unavailable and no cached weather exists for ZIP {zip_code}."
            ) from exc
